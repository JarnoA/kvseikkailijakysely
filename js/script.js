...
var app = {
    ...
    init: function() {
        ...
        if (path.indexOf('/en/') !== -1) this.state.lang = 'en';
        else if (path.indexOf('/sv/') !== -1) this.state.lang = 'sv';
        else this.state.lang = 'fi';
        
        console.log("App Initialized. Language:", this.state.lang);

        // Check for result in URL (safely)
        try {
            if (typeof URLSearchParams !== 'undefined') {
                var urlParams = new URLSearchParams(window.location.search);
                var sharedChar = urlParams.get('char');
                var self = this;
                if (sharedChar && quizData[this.state.lang].characters[sharedChar]) {
                    setTimeout(function() { self.showResult(sharedChar); }, 100);
                }
            }
        } catch (e) {
            console.error("URL Params not supported", e);
        }
    },
    ...
    startQuiz: function() {
        ...
        var charContainer = document.getElementById('avatar-container');
    ...
    resetScores: function() {
        for (var key in this.state.scores) this.state.scores[key] = 0;
    },

    renderQuestion: function() {
        var data = quizData[this.state.lang] || quizData['fi'];
        ...
        var percent = ((this.state.currentStep) / total) * 100;
        document.getElementById('progress-fill').style.width = percent + '%';

        var charContainer = document.getElementById('avatar-container');
        var sun = document.getElementById('sun');
        var clouds = document.getElementById('clouds');
        var mountBack = document.getElementById('mountains-back');
        var mountMid = document.getElementById('mountains-mid');

        var charPos = 15 + (this.state.currentStep * 15); 
        var cloudPos = this.state.currentStep * 30;
        var mountBackPos = this.state.currentStep * -40; 
        var mountMidPos = this.state.currentStep * -100; 
        var sunPos = this.state.currentStep * 60; 
        ...
        if (charContainer) {
            ...
            // Antennae animation
            var antL = document.getElementById('antenna-l');
            var antR = document.getElementById('antenna-r');
            if (antL && antR) {
                var angle = (this.state.currentStep % 2 === 0) ? 0 : -20;
    ...
    finishQuiz: function() {
        ...
        var winnerKey = this.calculateWinner();
    ...
    calculateWinner: function() {
        ...
        if (winners.indexOf('unelmoijakorento') !== -1 && winners.indexOf('osallisuuskimalainen') !== -1) return 'osallisuuskimalainen';
        
        // Rule 1: Fixed Ranking
        winners.sort(function(a, b) { return quizData['fi'].characters[a].rank - quizData['fi'].characters[b].rank; });
        return winners[0];
    },
    ...
    showResult: function(charKey) {
        ...
        for (var i = 0; i < classList.length; i++) {
            if (classList[i].indexOf('char-') === 0) classesToRemove.push(classList[i]);
        }
        // Use a standard for loop instead of forEach on the array, though forEach on array is safe in IE9+
        for (var j = 0; j < classesToRemove.length; j++) {
            document.body.classList.remove(classesToRemove[j]);
        }
        
        document.body.classList.add('result-page', 'char-' + charKey);
        var data = quizData[this.state.lang] || quizData['fi'];
        var charData = data.characters[charKey];
        if (!charData) return;

        document.getElementById('result-title').innerText = charData.title;
        document.getElementById('result-desc').innerText = charData.desc;
        
        var illuLabel = (this.state.lang === 'en') ? 'Illustration by:' : (this.state.lang === 'sv' ? 'Bild:' : 'Kuvitus:');
        var illuEl = document.getElementById('result-illustration');
        ...
        if (charContainer) {
            ...
            var svgBunny = document.getElementById('avatar-svg');
            if (svgBunny) svgBunny.style.display = 'none';
            
            var charImg = document.getElementById('char-result-img');
            if (!charImg) {
        ...
        try {
            ...
            if (qrContainer && typeof QRCode !== 'undefined') {
                ...
                var resultUrl = 'https://kvseikkailijakysely.fi/' + this.state.lang + '/tulokset/' + charKey + '.html';
    ...
    restart: function() {
        var rootPath = window.location.pathname.split('tulokset/')[0];
